set -e
cd /opt/disaster/channel-artifacts

# This script expedites the chaincode development process by automating the
# requisite channel create/join commands

# We use a pre-generated orderer.block and channel transaction artifact (disasterchannel.tx),
# both of which are created using the configtxgen tool

# first we create the channel against the specified configuration in disasterchannel.tx
# this call returns a channel configuration block - myc.block - to the CLI container
peer channel create -c disasterchannel -f ./channel.tx -o orderer:7050

# now we will join the channel and start the chain with disasterchannel.block serving as the
# channel's first block (i.e. the genesis block)
peer channel join -b ./disasterchannel.block
