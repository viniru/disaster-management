...
function printHelp () {
  echo "Usage: "
  echo "  disaster.sh up|down|restart|generate|reset|clean|upgrade|createneworg|startneworg|stopneworg [-c <channel name>] [-f <docker-compose-file>] [-i <imagetag>] [-o <logfile>] [-dev]"
  echo "  disaster.sh -h|--help (print this message)"
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"disasterchannel\")"
  ...
  echo
  echo "	disaster.sh generate -c disasterchannel"
  echo "	disaster.sh up -c disasterchannel -o logs/network.log"
  echo "        disaster.sh up -c disasterchannel -i 1.1.0-alpha"
  echo "	disaster.sh down -c disasterchannel"
  echo "        disaster.sh upgrade -c disasterchannel"
  echo
  echo "Taking all defaults:"
  echo "	disaster.sh generate"
  echo "	disaster.sh up"
  echo "	disaster.sh down"
}
...
function newOrgNetworkUp () {
  checkPrereqs
  # generate artifacts if they don't exist
  if [ ! -d "crypto-config/peerOrganizations/exportingentityorg.disaster.com" ]; then
...
function upgradeNetwork () {
  docker inspect  -f '{{.Config.Volumes}}' orderer.disaster.com |grep -q '/var/hyperledger/production/orderer'
  if [ $? -ne 0 ]; then
  ...
  docker-compose $COMPOSE_FILES stop orderer.disaster.com
  docker cp -a orderer.disaster.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.disaster.com
  docker-compose $COMPOSE_FILES up --no-deps orderer.disaster.com

  for PEER in peer0.reliefcamp.disaster.com peer0.localhub.disaster.com peer0.logistics.disaster.com peer0.ngo.disaster.com peer0.government.disaster.com; do
...
function newOrgNetworkDown () {
  ...
  for PEER in peer0.exportingentityorg.disaster.com; do
    # Remove any old containers and images for this peer
    CC_CONTAINERS=$(docker ps -a | grep dev-$PEER | awk '{print $1}')
    if [ -n "$CC_CONTAINERS" ] ; then
      docker rm -f $CC_CONTAINERS
...
function networkClean () {
  ...
  #rm -f docker-compose-e2e.yaml add_org/docker-compose-exportingEntityOrg.yaml #this line was originally not commented
  # remove client certs 
  rm -rf client-certs
  if [ "$DEV_MODE" = true ] ; then
     popd
...
function replacePrivateKey () {
  ...
  if [ "$DEV_MODE" = true ] ; then
    CURRENT_DIR=$PWD
    cd crypto-config/peerOrganizations/devorg.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    ...
    CURRENT_DIR=$PWD
    cd crypto-config/peerOrganizations/reliefcamp.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/RELIEFCAMP_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/localhub.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/LOCALHUB_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/logistics.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/LOGISTICS_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/ngo.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/NGO_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
    cd crypto-config/peerOrganizations/government.disaster.com/ca/
    PRIV_KEY=$(ls *_sk)
    cd "$CURRENT_DIR"
    sed -i "s/GOVERNMENT_CA_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  fi
}
...
function replacePrivateKeyForNewOrg () {
  ...
  # actual values of the private key file names for the two CAs.
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/exportingentityorg.disaster.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
function generateCertsForNewOrg (){
  ...
  if [ -d "crypto-config/peerOrganizations/exportingentityorg.disaster.com" ]; then
    rm -Rf crypto-config/peerOrganizations/exportingentityorg.disaster.com
  fi
  set -x
...
# for the sample network. There are five members - one Orderer Org (``DisasterOrdererOrg``)
# and four Peer Orgs (``ExporterOrg``, ``ImporterOrg``, ``CarrierOrg`` & ``RegulatorOrg``)
# each managing and maintaining one peer node.
# This file also specifies a consortium - ``DisasterConsortium`` - consisting of our
# four Peer Orgs.  Pay specific attention to the "Profiles" section at the top of
# this file.  You will notice that we have two unique headers. One for the orderer genesis
# block - ``FourOrgsDisasterOrdererGenesis`` - and one for our channel - ``FourOrgsDisasterChannel``.
# These headers are important, as we will pass them in as arguments when we create
# our artifacts.  This file also contains two additional specifications that are worth
# noting.  Firstly, we specify the anchor peers for each Peer Org
# (``peer0.exporterorg.disaster.com`` & ``peer0.importerorg.disaster.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo "###########################################################"
  if [ "$DEV_MODE" = true ] ; then
    PROFILE=OneOrgDisasterOrdererGenesis
    CHANNEL_PROFILE=OneOrgDisasterChannel
  else 
    PROFILE=DisasterOrdererGenesis
    CHANNEL_PROFILE=DisasterManagementChannel
  fi
  ...
  if [ "$DEV_MODE" = false ] ; then
    ...
    echo "#####################################################################"
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate ./channel-artifacts/ReliefCampMSPanchors.tx -channelID $CHANNEL_NAME -asOrg ReliefCampMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    ./channel-artifacts/LocalHubMSPanchors.tx -channelID $CHANNEL_NAME -asOrg LocalHubMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/LogisticsMSPanchors.tx -channelID $CHANNEL_NAME -asOrg LogisticsMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/NGOMSPanchors.tx -channelID $CHANNEL_NAME -asOrg NGOMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
    ...
    set -x
    configtxgen -profile $CHANNEL_PROFILE -outputAnchorPeersUpdate \
    ./channel-artifacts/GovernmentMSPanchors.tx -channelID $CHANNEL_NAME -asOrg GovernmentMSP
    res=$?
    set +x
    if [ $res -ne 0 ]; then
      echo "Failed to generate anchor peer update for CarrierOrgMSP..."
...
if [ "${MODE}" == "up" ]; then
  ...
  generateCerts
  echo "**********************************************************************************************************************************"
  echo "gnerating certificates done"
  echo "**********************************************************************************************************************************"
  replacePrivateKey
  echo "**********************************************************************************************************************************"
  echo "replacing private key done"
  echo "**********************************************************************************************************************************"
  generateChannelArtifacts
  echo "**********************************************************************************************************************************"
  echo "replacing private key done"
